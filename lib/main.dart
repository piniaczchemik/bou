...
class ThemeController extends InheritedWidget {
  ...
  const ThemeController({super.key, required this.mode, required this.toggle, required Widget child}) : super(child: child);
...
class ExercisePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final workouts = [
      const WorkoutTile(name: 'Flat Bench Press', sets: '4 x 6-8', rest: '120s'),
      const WorkoutTile(name: 'Incline DB Press', sets: '4 x 8-10', rest: '90s'),
      const WorkoutTile(name: 'Cable Fly (Mid)', sets: '3 x 12-15', rest: '60s'),
      const WorkoutTile(name: 'Rotator Cuff (Cuban Press)', sets: '3 x 12', rest: '60s'),
    ];
          ...
          children: [
            ...
            Expanded(
              child: ListView.separated(
                ...
                itemBuilder: (context, i) => workouts[i],
              ),
            ),
            PrimaryButton(label: 'Start Session', icon: Icons.timer_outlined, onPressed: () {}),
          ],
        ),
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    return Text(
      ...
      style: GoogleFonts.inter(
        fontSize: 16,
        fontWeight: FontWeight.w700,
        color: scheme.onBackground.withOpacity(.9),
      ),
    );
  }
}
